 ...
 *  You may freely license this file under the MIT-x11, or the 2- or 3- or New
 * BSD Licenses. You may also use this as though it is public domain.
 *
 ...
 *   3: Define MICROGDBSTUB_IMPLEMENTATION at least in one place this is
 * included in your program. 4: If you want to let this manage the server as a
 * network device, simply #define MICROGDBSTUB_SOCKETS
 *
...
int RVWriteFlash(void *dev, uint32_t memaddy, uint32_t length,
                 uint8_t *payload);
...
int MicroGDBStubHandleClientData(void *dev, const uint8_t *rxdata, int len);
...
static int StringMatch(const char *haystack, const char *mat) {
  ...
  for (i = 0; mat[i] && haystack[i]; i++)
    if (mat[i] != haystack[i] || haystack[i] == 0)
      break;
  return mat[i] == 0;
}
...
int HandleGDBPacket(void *dev, char *data, int len) {
  int i;
  char *odata = data;
  ...
  case 'q':
    if (StringMatch(data, "Attached")) {
      ...
      SendReplyFull("PacketSize=f000;qXfer:memory-map:read+");
                                     ...
                                     // 0.  Those are special)
      SendReplyFull("QC1");
    else if (StringMatch(data, "fThreadInfo")) // Query all active thread IDs
                                               // (Can't be 0 or 1)
      SendReplyFull("m1");
    else if (StringMatch(
                 data, "sThreadInfo")) // Query all active thread IDs, continued
      SendReplyFull("l");
    ...
    else if (StringMatch(data, "Rcmd,7265736574")) // "monitor reset"
    {
      ...
      char map[mslen];
      struct InternalState *iss =
          (struct InternalState *)(((struct ProgrammerStructBase *)dev)
                                       ->internal);
  ...
  {
    uint32_t reg, value;
    if (ReadHex(&data, -1, &reg) < 0)
      goto err;
    if (*(data++) != ',')
      goto err;
    if (ReadHex(&data, -1, &value) < 0)
      goto err;
    printf("REG: %02x = %08x\n", reg, value);
  ...
  case 'z': // unset
  {
    ...
    uint32_t addr = 0;
    uint32_t time = 0;
    if (ReadHex(&data, -1, &type) < 0)
      goto err;
    if (*(data++) != ',')
      goto err;
    if (ReadHex(&data, -1, &addr) < 0)
      goto err;
    if (*(data++) != ',')
      goto err;
    if (ReadHex(&data, -1, &time) < 0)
      goto err;
  ...
  case 'm': {
    ...
    if (ReadHex(&data, -1, &address_to_read) < 0)
      goto err;
    if (*(data++) != ',')
      goto err;
    if (ReadHex(&data, -1, &length_to_read) < 0)
      goto err;
  ...
  case 'M': {
    ...
    if (ReadHex(&data, -1, &address_to_write) < 0)
      goto err;
    if (*(data++) != ',')
      goto err;
    if (ReadHex(&data, -1, &length_to_write) < 0)
      goto err;
    if (*(data++) != ':')
      goto err;
    ...
    for (i = 0; i < length_to_write; i++) {
      uint32_t rv;
      if (ReadHex(&data, 2, &rv) < 0)
        goto err;
      meml[i] = rv;
    }
    if (RVWriteRAM(dev, address_to_write, length_to_write, meml) < 0)
      goto err;
    SendReplyFull("OK");
    break;
  }
  case 'X': {
    ...
    if (*(data++) != ':')
      goto err;
    if (RVWriteRAM(dev, address_to_write, length_to_write, (uint8_t *)data) < 0)
      goto err;
    SendReplyFull("OK");
    break;
  }
  case 'v':
    ...
    } else if (StringMatch(data, "FlashErase")) // vFlashErase
    {
      ...
      if (ReadHex(&data, -1, &address_to_erase) < 0)
        goto err;
      if (*(data++) != ',')
        goto err;
      if (ReadHex(&data, -1, &length_to_erase) < 0)
        goto err;
  ...
  case 'g': {
    ...
    for (i = 0; i < 17; i++) {
      uint32_t regret;
      if (RVReadCPURegister(dev, i, &regret))
        goto err;
  ...
  case 'p': {
    ...
    if (ReadHex(&data, 2, &regno) < 0)
      ...
      char cts[9];
      uint32_t regret;
      if (RVReadCPURegister(dev, regno, &regret))
        goto err;
  ...
  for (pl = 0; pl < len; pl++) {
    ...
    case 3: {
      ...
      if (gdbrunningcsum == gdbchecksum) {
        ...
        if (HandleGDBPacket(dev, (char *)gdbbuffer, gdbbufferplace)) {
          return -1;
        }
      } else {
...
void MicroGDBStubHandleDisconnect(void *dev) { RVHandleDisconnect(dev); }
...
static int GDBListen(void *dev) {
  ...
  lx.l_onoff = 1;
  lx.l_linger = 0;
  setsockopt(serverSocket, SOL_SOCKET, SO_LINGER, (const char *)&lx,
             sizeof(lx));

  // Enable SO_REUSEADDR
  int reusevar = 1;
  setsockopt(serverSocket, SOL_SOCKET, SO_REUSEADDR, (const char *)&reusevar,
             sizeof(reusevar));
#else
  struct linger lx;
  lx.l_onoff = 1;
  ...
  // Enable SO_REUSEADDR
  int reusevar = 1;
  setsockopt(serverSocket, SOL_SOCKET, SO_REUSEADDR, &reusevar,
             sizeof(reusevar));
#endif
...
int MicroGDBPollServer(void *dev) {
  ...
  if (allpolls[0].revents & POLLIN) {
    if (listenMode == 1) {
      ...
      int tsocket =
          accept(serverSocket, (struct sockaddr *)&tin, (void *)&addrlen);
      closesocket(serverSocket);
