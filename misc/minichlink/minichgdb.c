...
uint8_t
    software_breakpoint_type[MAX_SOFTWARE_BREAKPOINTS]; // 0 = not in use, 1 =
                                                        // 32-bit, 2 = 16-bit.
...
void RVNetPoll(void *dev) {
  if (!MCF.ReadReg32) {
    fprintf(stderr, "Error: Can't poll GDB because no ReadReg32 supported on "
                    "this programmer\n");
    return;
  }
...
int RVReadCPURegister(void *dev, int regno, uint32_t *regret) {
  ...
  if (regno == 32)
    regno = 16; // Hack - Make 32 also 16 for old GDBs.
  if (regno > 16)
...
int RVWriteCPURegister(void *dev, int regno, uint32_t value) {
  ...
  if (!MCF.WriteAllCPURegisters) {
    fprintf(stderr, "ERROR: MCF.WriteAllCPURegisters is not implemented on "
                    "this platform\n");
    return -99;
  }
...
void RVDebugExec(void *dev, int halt_reset_or_resume) {
  ...
  if (halt_reset_or_resume == 4 || halt_reset_or_resume == 2) {
    ...
    for (i = 0; i < MAX_SOFTWARE_BREAKPOINTS; i++) {
      if (exceptionptr == software_breakpoint_addy[i] &&
          software_breakpoint_type[i]) {
        matchingbreakpoint = i;
      }
    }

    if (matchingbreakpoint >= 0) {
      // This is a known breakpoint.  Need to set it back.  Single Step.  Then
      // continue.
...
static int InternalClearFlashOfSoftwareBreakpoint(void *dev, int i) {
  ...
  if (software_breakpoint_type[i] == 1) {
    // 32-bit instruction
    r = MCF.WriteBinaryBlob(dev, software_breakpoint_addy[i], 4,
                            (uint8_t *)&previous_word_at_breakpoint_address[i]);
  } else {
    // 16-bit instruction
    r = MCF.WriteBinaryBlob(dev, software_breakpoint_addy[i], 2,
                            (uint8_t *)&previous_word_at_breakpoint_address[i]);
  }
...
int PollGDBServer(void *dev) { return MicroGDBPollServer(dev); }

void ExitGDBServer(void *dev) { MicroGDBExitServer(dev); }

int SetupGDBServer(void *dev) { return MicroGDBStubStartup(dev); }
